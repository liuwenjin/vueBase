...
<script>
import Example1 from "../../../../static/example/funnelChart/1.vue";
...
export default {
  ...
  data() {
    return {
      basicData: {},
      componentPath: "/static/example/funnelChart/1.vue",
      exampleSelect: "",
      activeNames: "1",
      exampleCodeDialog: {
        visible: false,
        ...
        data: [
          {
            name: "click",
            param: "data",
            description: "当点击漏斗图某块区域时触发该事件",
          },
          {
            name: "mouseover",
            param: "data",
            description: "当鼠标漏斗图上某块区域触发该事件",
          }
        ],
      },
      paramDescription: {
  ...
  methods: {
    ...
    exampleCallback1() {
      ...
      tData.itemStyle = {sort: 'ascending'}
      this.updateExampleData(tData);
    },
    exampleCallback2() {
      var tData = { ...this.basicData };
      tData.itemStyle = {sort: 'descending'}
      this.updateExampleData(tData);
    },
    dealSelectExample(v) {
      console.log(v);
