...
Utils = {
    ...
    addPlace: function(name, resultArr) {
        resultArr = [];
        if (name.constructor.name === "Array") {
            for (var i = 0; i < name.length; i++) {
                this.addPlace(name[i], resultArr);
            }
        } else {
            webCpu.cards.baiduMapItem.task.updatePosition({ name: name }, function(d) {
                var item = d.Hr[0];
                console.log(item.point, item.point.lat, item.point.lng);
                if (resultArr && resultArr.push) {
                    resultArr.push({ name: name, pos: item.point });
                }
            });
        }
